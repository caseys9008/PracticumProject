...
It looks for a .pdb file in the path

python /Users/tabinkowski/pdb/metrics.py path path 

Exmple: 
python /Users/tabinkowski/pdb/metrics.py 301_278-312_a 301_278-312_a

There is a 301_278-312_a.pdb file in the directory that it will find.
'''
...
def get_metrics_for_one_pose(pose, structure_name):
    ...
    #d_of_data['num_backrub_ensemble_buhs'] = [(structure_name, len(buried_unsat_donors) + len(buried_unsat_acceptors))]
    #d_of_data['average_num_backrub_ensemble_buhs'] = [(structure_name, (len(buried_unsat_donors) + len(buried_unsat_donors)) / pose.size())]

    # Only count heavy atom unsats to compare with the default buried unsat calculator
    d_of_data['num_backrub_ensemble_buhs'] = [(structure_name, len(buried_unsat_acceptors))]
    d_of_data['average_num_backrub_ensemble_buhs'] = [(structure_name, len(buried_unsat_acceptors) / pose.size())]
